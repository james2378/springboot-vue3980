		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','user') || $check_field('add','user') || $check_field('set','user')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="用户" prop="user">
						<el-select v-if="user_group === '管理员' || (form['use_seat_information_id'] && $check_field('set','user')) || (!form['use_seat_information_id'] && $check_field('add','user'))" id="user" v-model="form['user']" :disabled="disabledObj['user_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form['use_seat_information_id'] && $check_field('set','full_name')) || (!form['use_seat_information_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','contact_information') || $check_field('add','contact_information') || $check_field('set','contact_information')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="联系方式" prop="contact_information">
					<el-input id="contact_information" v-model="form['contact_information']" placeholder="请输入联系方式"
							  v-if="user_group === '管理员' || (form['use_seat_information_id'] && $check_field('set','contact_information')) || (!form['use_seat_information_id'] && $check_field('add','contact_information'))" :disabled="disabledObj['contact_information_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','name_of_study_room') || $check_field('add','name_of_study_room') || $check_field('set','name_of_study_room')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="自习室名称" prop="name_of_study_room">
					<el-input id="name_of_study_room" v-model="form['name_of_study_room']" placeholder="请输入自习室名称"
							  v-if="user_group === '管理员' || (form['use_seat_information_id'] && $check_field('set','name_of_study_room')) || (!form['use_seat_information_id'] && $check_field('add','name_of_study_room'))" :disabled="disabledObj['name_of_study_room_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','seat_type') || $check_field('add','seat_type') || $check_field('set','seat_type')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="座位类型" prop="seat_type">
					<el-input id="seat_type" v-model="form['seat_type']" placeholder="请输入座位类型"
							  v-if="user_group === '管理员' || (form['use_seat_information_id'] && $check_field('set','seat_type')) || (!form['use_seat_information_id'] && $check_field('add','seat_type'))" :disabled="disabledObj['seat_type_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','appointment_period') || $check_field('add','appointment_period') || $check_field('set','appointment_period')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="预约时段" prop="appointment_period">
					<el-input id="appointment_period" v-model="form['appointment_period']" placeholder="请输入预约时段"
							  v-if="user_group === '管理员' || (form['use_seat_information_id'] && $check_field('set','appointment_period')) || (!form['use_seat_information_id'] && $check_field('add','appointment_period'))" :disabled="disabledObj['appointment_period_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','number_of_appointments') || $check_field('add','number_of_appointments') || $check_field('set','number_of_appointments')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="预约人数" prop="number_of_appointments">
					<el-input-number id="number_of_appointments" v-model.number="form['number_of_appointments']"
						v-if="user_group === '管理员' || (form['use_seat_information_id'] && $check_field('set','number_of_appointments')) || (!form['use_seat_information_id'] && $check_field('add','number_of_appointments'))"></el-input-number>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','administrator_confirmation') || $check_field('add','administrator_confirmation') || $check_field('set','administrator_confirmation')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="管理员确认" prop="administrator_confirmation">
					<el-input id="administrator_confirmation" v-model="form['administrator_confirmation']" placeholder="请输入管理员确认"
							  v-if="user_group === '管理员' || (form['use_seat_information_id'] && $check_field('set','administrator_confirmation')) || (!form['use_seat_information_id'] && $check_field('add','administrator_confirmation'))" :disabled="disabledObj['administrator_confirmation_isDisabled']"></el-input>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "use_seat_information_id",
				url_add: "~/api/use_seat_information/add?",
				url_set: "~/api/use_seat_information/set?",
				url_get_obj: "~/api/use_seat_information/get_obj?",
				url_upload: "~/api/use_seat_information/upload?",

				query: {
					"use_seat_information_id": 0,
				},
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/use_seat_information/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/use_seat_information/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/use_seat_information/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/use_seat_information/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/use_seat_information/view','get');
