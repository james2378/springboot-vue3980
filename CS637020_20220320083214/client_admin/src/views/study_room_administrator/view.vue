<template>
	<el-main class="bg">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','merchant_account_number') || $check_field('add','merchant_account_number') || $check_field('set','merchant_account_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="商家账号" prop="merchant_account_number">
					<el-input id="merchant_account_number" v-model="form_sub['merchant_account_number']" placeholder="请输入商家账号"
							  v-if="user_group === '管理员' || (form_sub['study_room_administrator_id'] && $check_field('set','merchant_account_number')) || (!form_sub['study_room_administrator_id'] && $check_field('add','merchant_account_number'))"></el-input>
					<div v-else-if="$check_field('get','merchant_account_number')">{{form_sub['merchant_account_number']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form_sub['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form_sub['study_room_administrator_id'] && $check_field('set','full_name')) || (!form_sub['study_room_administrator_id'] && $check_field('add','full_name'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['study_room_administrator_id'] && $check_field('set','gender')) || (!form_sub['study_room_administrator_id'] && $check_field('add','gender'))">
	...
	export default {
		...
		data() {
			return {
				...
				group_table: "study_room_administrator",
				is_password: true,
				...
				disabledObj:{
					"merchant_account_number_isDisabled": false,
					"full_name_isDisabled": false,
					"gender_isDisabled": false,
				},

				form_sub: {
					"merchant_account_number":'', // 商家账号
					"full_name":'', // 姓名
					"gender":'', // 性别
				    "user_id": 0,
					"study_room_administrator_id": 0 // ID
				},
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["study_room_administrator_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?study_room_administrator_id=" + form_sub["study_room_administrator_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/study_room_administrator/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/study_room_administrator/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/study_room_administrator/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/study_room_administrator/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/study_room_administrator/view','get');
